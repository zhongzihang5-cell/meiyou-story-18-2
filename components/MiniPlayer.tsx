...
const DEFAULT_TRACKS: MiniPlayerTrack[] = [
  { key: '001', href: '/player/001', title: '小星星说话了', cover_emoji: '🌟', cover_bg: 'linear-gradient(135deg,#FFF8E1,#FFE0B2)', duration_sec: 157, type: '故事' },
  { key: '026', href: '/player/026', title: '叮咚叮咚的声音', cover_emoji: '🎵', cover_bg: 'linear-gradient(135deg,#E8F5E9,#C8E6C9)', duration_sec: 120, type: '故事' },
  { key: '003', href: '/player/003', title: '妈妈的怀抱', cover_emoji: '🌸', cover_bg: 'linear-gradient(135deg,#FCE4EC,#F8BBD0)', duration_sec: 194, type: '故事' },
  { key: '051', href: '/player/051', title: '小皮球在哪里', cover_emoji: '🔴', cover_bg: 'linear-gradient(135deg,#E0F7FA,#B2EBF2)', duration_sec: 157, type: '故事' },
  { key: '076', href: '/player/076', title: '小熊学刷牙', cover_emoji: '🐻', cover_bg: 'linear-gradient(135deg,#E8F5E9,#C8E6C9)', duration_sec: 231, type: '故事' },
]

type MiniPlayerProps = {
  tracks?: MiniPlayerTrack[]
  /** 与外部播放入口同步高亮条（如亲声讲首页卡片） */
  syncKey?: string | null
}

export default function MiniPlayer({ tracks: tracksProp, syncKey }: MiniPlayerProps) {
  const tracks = tracksProp ?? DEFAULT_TRACKS
  const [playing, setPlaying] = useState(false)
  const [showHistory, setShowHistory] = useState(false)
  const router = useRouter()
  const [currentTrackIdx, setCurrentTrackIdx] = useState(0)
  const current = tracks[currentTrackIdx] ?? tracks[0]

  useEffect(() => {
    if (syncKey == null || syncKey === '') return
    const i = tracks.findIndex(t => t.key === syncKey)
    if (i >= 0) setCurrentTrackIdx(i)
  }, [syncKey, tracks])
    ...
    <>
      ...
      <div
        ...
        <div style={{ display: 'flex', alignItems: 'center', padding: '10px 16px 14px', gap: 12 }}>
          ...
          {/* 标题+类型 */}
          <div style={{ flex: 1, minWidth: 0, cursor: 'pointer' }} onClick={() => router.push(current.href)}>
            <div
              style={{
                color: '#fff',
                fontSize: 13,
          ...
          <button
            onClick={() => {
              setCurrentTrackIdx(idx => (idx < tracks.length - 1 ? idx + 1 : idx))
            }}
            style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 4, color: 'rgba(255,255,255,0.7)' }}>
            <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
              ...
              {tracks.map((track, i) => (
                <div
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div
                      style={{
                        color: i === currentTrackIdx ? '#F06292' : '#fff',
                        fontSize: 14,
                        fontWeight: i === currentTrackIdx ? 700 : 500,
                        whiteSpace: 'nowrap',
                        overflow: 'hidden',
                        textOverflow: 'ellipsis',
                      }}>
